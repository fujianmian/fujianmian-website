...
export function ResumeStrip({ children, hintId, className = "" }) {
  ...
  return (
    <div
      ...
      className={`resume-strip mt-7 overflow-x-visible lg:mt-8 lg:overflow-x-auto lg:overscroll-x-contain ${className}`}
    >
      <div className="border-y border-slate-800 lg:flex lg:h-full lg:w-max lg:min-w-full lg:border-l">
        {children}
      </div>
    </div>
  );
